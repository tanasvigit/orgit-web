...
function CalendarCountColumn({ label, title, count }: { label: string; title?: string; count: number }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 px-0.5 py-0.5">
      <span
        title={title}
      ...
      <span
        className={`flex min-h-[12px] items-center justify-center text-[10px] font-bold tabular-nums leading-none ${
...
export function DashboardCalendarDayBadge({ selfCount, assignedCount }: Props) {
  ...
  return (
    <div
      className="mt-1 flex overflow-hidden rounded border border-primary/30 bg-primary/5 dark:bg-primary/10"
      role="group"
